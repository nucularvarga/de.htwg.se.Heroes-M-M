...
class MapCommand(Map: MapMode, e: Event, controller: Controller) extends Command {
  ...
  override def undoStep: Unit = controller.mode = controller.mode.handle(inverseEvent(e))
  ...
  def inverseEvent(d: Event): Event = {
    d match {
      ...
      case Event.MoveLeft => Event.MoveLeft
      case Event.MoveDown => Event.MoveUp
      case Event.MoveRight => Event.MoveRight
    }
  }
