...
class PlayerSpec extends WordSpec with Matchers {
  ...
  "A Player" when {
    "not set to any value " should {
      val emptyPlayer = Player("", 0 , 0, 0, 0, 0)
      "have value '' and 0" in {
    ...
    "set to a specific values" should {
      val nonEmptyPlayer = Player("test", 100, 10, 0, 1, 1)
      ...
      "player walks right" in {
        nonEmptyPlayer.walk(1, 0) should be(Player("test", 100, 10, 0, 2, 1))
      }
      "player grows stronger" in {
        nonEmptyPlayer.powerUp(200) should be(Player("test", 100, 210, 0, 1, 1))
      }
    }
  }
}
